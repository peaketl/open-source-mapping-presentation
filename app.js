...
// Fetch case list from Martin, then init the map
...
function updateStats(cases) {
    ...
    document.getElementById("stat-solved").textContent = cases.filter(c => c.solved).length;
    document.getElementById("stat-unsolved").textContent = cases.filter(c => !c.solved).length;
}
